...
### Updated Features
* Added a new attribute, ```updated```, which is a unix timestamp for when the video was last updated in the JW Platform API. [/videos/list](https://developer.jwplayer.com/jw-platform/reference/v1/methods/videos/list.html) calls can ```order_by``` (```updated:desc``` or ```updated:asc```) or filtered by the ```updated_after``` parameter.
    * The attribute is updated whenever:
        ...
        * When a tag is globally deleted (via the dashboard or [/accounts/tags/delete/](https://developer.jwplayer.com/jw-platform/reference/v1/methods/accounts/tags/    delete.html)) all media associated with that tag gets a new updated value
...
### Updated Features
* Per item data driven recommendations are now available for content with URL token signing enforcement enabled.
...
### New Features
* Designate videos as 360 content. You can now designate a video as 360 including the stereoscopic mode from the video detail page or by adding the custom parameter ```stereomode``` with a value of ```monoscopic```, ```stereoscopicTopBottom``` or```stereoscopicLeftRight```. This will tell the player to use 360 rendering with the appropriate 3D mode. 360 formatted videos can be uploaded, transcoded and served just like standard videos.
### Updated Features
...
* Sharing embed codes use protocol relative URLS for sharing over http or https with the same code.
